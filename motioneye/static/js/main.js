...
function initUI() {
    ...
    $('#cleanCloudEnabledSwitch').change(function () {
        ...
        if (enabled) {
            runAlertDialog(('Isso removerá recursivamente todos os arquivos presentes na pasta da nuvem "' + folder +
                    '", não apenas aqueles enviados pelo sistema!'));
        }
    });
    ...
    preserveSelects.focus(function () {
        ...
        if (value != '0' && this._prevValue == '0') {
            ...
            runAlertDialog(('Isso removerá recursivamente todos os arquivos de mídia antigos presentes no diretório "' + rootDir +
                    '", não apenas aqueles criados pelo sistema!'));
        }
    });
...
function dict2CameraUi(dict) {
    ...
    switch (dict['proto']) {
        case 'v4l2':
            prettyType = 'V4L2 Câmera';
            break;

        case 'netcam':
            prettyType = 'Câmera IP';
            break;

        case 'mmal':
            prettyType = 'MMAL Câmera';
            break;

        case 'motioneye':
            prettyType = 'Sistema Remoto Infotec';
            break;

        case 'mjpeg':
            prettyType = 'Simples MJPEG Câmera';
            break;
    }
    ...
    if (dict['available_resolutions']) {
        ...
        $('#resolutionSelect').append('<option value="custom">Customizar</option>');
    }
    ...
    var content =
            $('<table class="restore-dialog">' +
                '<tr>' +
                    ...
                    '<td><span class="help-mark" title="o arquivo de backup que você baixou anteriormente">?</span></td>' +
                '</tr>' +
            '</table>');
...
function runPictureDialog(entries, pos, mediaType, onDelete) {
    ...
    var prevArrow = $('<div class="picture-dialog-prev-arrow button mouse-effect" title="Anterior"></div>');
    ...
    var nextArrow = $('<div class="picture-dialog-next-arrow button mouse-effect" title="Próxima"></div>');
...
function runAddCameraDialog() {
    if (Object.keys(pushConfigs).length) {
        return runAlertDialog('Por favor. aplique as modificações primeiro!');
    }
            ...
            $('<table class="add-camera-dialog">' +
                '<tr>' +
                    ...
                    '<td class="dialog-item-value"><select class="styled" id="typeSelect">' +
                        ...
                        (hasNetCamSupport ? '<option value="netcam">Camera IP (rtsp://) </option>' : '') +
                        '<option value="motioneye">Outro sistema INFOTEC</option>' +
                        '<option value="mjpeg">MJPEG Camera</option>' +
                    '</select></td>' +
                    '<td><span class="help-mark" title="o tipo de câmera que você deseja adicionar">?</span></td>' +
                '</tr>' +
                '<tr class="motioneye netcam mjpeg">' +
                    '<td class="dialog-item-label"><span class="dialog-item-label">Endereço da Câmera</span></td>' +
                    '<td class="dialog-item-value"><input type="text" class="styled" id="urlEntry" placeholder="http://exemplo.com:8765/cams/..."></td>' +
                    '<td><span class="help-mark" title="Insira a URL (ex. http://example.com:8080/cam/)">?</span></td>' +
                '</tr>' +
                '<tr class="motioneye netcam mjpeg">' +
                    '<td class="dialog-item-label"><span class="dialog-item-label">Usuario</span></td>' +
                    '<td class="dialog-item-value"><input type="text" class="styled" id="usernameEntry" placeholder="Seu usuario..."></td>' +
                    '<td><span class="help-mark" title="Usuario requerido (ex. admin)">?</span></td>' +
                '</tr>' +
                '<tr class="motioneye netcam mjpeg">' +
                    '<td class="dialog-item-label"><span class="dialog-item-label">Senha</span></td>' +
                    '<td class="dialog-item-value"><input type="password" class="styled" id="passwordEntry" placeholder="Senha."></td>' +
                    '<td><span class="help-mark" title="Senha Requerida">?</span></td>' +
                '</tr>' +
                '<tr class="v4l2 motioneye netcam mjpeg mmal">' +
                    ...
                    '<td><span class="help-mark" title="a câmera que você deseja adicionar">?</span></td>' +
                '</tr>' +
                '<tr class="v4l2 motioneye netcam mjpeg mmal">' +
    ...
    function updateUi() {
        ...
        if (typeSelect.val() == 'motioneye') {
            ...
            addCameraInfo.html(
                    'As câmeras remotas da Infotec são câmeras instaladas atrás de outro servidor de monitoramento da Infotec. ' +
                    'Adicioná-los aqui permitirá que você os visualize e gerencie remotamente.');
        }
            ...
            addCameraInfo.html(
                    'Câmeras IP são dispositivos que transmitem nativamente vídeos RTSP / RTMP ou MJPEG ou imagens JPEG simples. ' +
                    "Geralmente são usados os endereços (rtsp://000.000.000.0:554/user=admin_password=_channel=1_stream=0.sdp) em cameras mais baratas.");
        }
        else if (typeSelect.val() == 'mmal') {
            content.find('tr.mmal').css('display', 'table-row');
            addCameraInfo.html(
                    'As câmeras MMAL locais são dispositivos conectados diretamente ao seu sistema. ' +
                    'Geralmente, são câmeras específicas da placa, caso usado em um Raspberry.');
        }
            ...
            addCameraInfo.html(
                    'Adicionar seu dispositivo como uma câmera MJPEG simples, e não como uma câmera de rede, melhorará a taxa de quadros, ' +
                    'mas não haverá detecção de movimento, captura de imagem ou gravação de filme. ' +
                    'A câmera deve estar acessível ao servidor e ao navegador. ' +
                    'Este tipo de câmera não é compatível com o Internet Explorer.');
        }
        else { /* assuming v4l2 */
            content.find('tr.v4l2').css('display', 'table-row');
            addCameraInfo.html(
                    'As câmeras V4L2 locais são dispositivos de câmera conectados diretamente ao seu sistema Infotec monitoramento, ' +
                    'via USB.');
        }
    ...
    function listCameras() {
        ...
        ajax('GET', basePath + 'config/list/', data, function (data) {
            ...
            if (!data.cameras || !data.cameras.length) {
                addCameraSelect.append('<option value="">(Sem câmeras)</option>');
            }
...
function runTimelapseDialog(cameraId, groupKey, group) {
            ...
            $('<table class="timelapse-dialog">' +
                ...
                '<tr>' +
                    ...
                    '<td class="dialog-item-value">' +
                        ...
                        '</select>' +
                    '</td>' +
                    '<td><span class="help-mark" title="escolha o intervalo de tempo entre duas fotos selecionadas">?</span></td>' +
                '</tr>' +
                '<tr>' +
                    '<td class="dialog-item-label"><span class="dialog-item-label">Taxa de quadros</span></td>' +
                    '<td class="dialog-item-value"><input type="text" class="styled range" id="framerateSlider"></td>' +
                    '<td><span class="help-mark" title="escolha a rapidez com que deseja que a reprodução em timelapse seja">?</span></td>' +
                '</tr>' +
            '</table>');
    ...
    if (group.length > 1440) { /* one day worth of pictures, taken 1 minute apart */
        timelapseWarning.html('Dado o grande número de fotos, a criação do timelapse pode demorar um pouco!');
            ...
            function checkTimelapse() {
                ...
                ajax('GET', actualUrl, data, function (data) {
                    ...
                    if (data.progress != -1 && first) {
                        showPopupMessage('Um timelapse já está sendo criado.');
                    }

                    if (data.progress == -1 && !first && !data.key) {
                        hideModalDialog(); /* progress */
                        hideModalDialog(); /* timelapse dialog */
                        showErrorMessage('Não foi possível criar o filme timelapse.');
                        return;
                    }
...
function runMediaDialog(cameraId, mediaType) {
        ...
        function addEntries() {
            ...
            entries.forEach(function (entry) {
                ...
                if (!entryDiv) {
                    ...
                    var deleteButton = $('<div class="media-list-delete-button button">Deletar</div>');
                    if (isAdmin()) {
    ...
    if (mediaType == 'picture') {
        var zippedButton = $('<div class="media-dialog-button">Compactar</div>');
    ...
    if (isAdmin()) {
        var deleteAllButton = $('<div class="media-dialog-button media-dialog-delete-all-button">Deletar tudo</div>');
    ...
    ajax('GET', basePath + mediaType + '/' + cameraId + '/list/', null, function (data) {
        ...
        if (keys.length) {
            ...
            groupsDiv.html('(Sem arquivos por aqui... )');
            mediaListDiv.remove();
        }
                ...
                '<div class="camera-overlay">' +
                    '<div class="camera-overlay-top">' +
                        ...
                        '<div class="camera-top-buttons">' +
                            ...
                            '<div class="button icon camera-top-button mouse-effect media-pictures" title="Abrir Imagens no navegador"></div>' +
                            '<div class="button icon camera-top-button mouse-effect media-movies" title="Abrir Videos no navegador"></div>' +
                            '<div class="button icon camera-top-button mouse-effect configure" title="Configurar Câmera"></div>' +
                        '</div>' +
                    '</div>' +
                    '<div class="camera-overlay-mask"></div>' +
                    '<div class="camera-overlay-bottom">' +
                        '<div class="camera-info">' +
                            '<span class="camera-info" title="FPS Câmera/FPS de Gravação"></span>' +
                        '</div>' +
                        ...
                        '<div class="camera-action-buttons-wrapper">' +
                                ...
                                '<div class="button icon camera-action-button mouse-effect record-start" title="alternar para o modo de gravação contínua"></div>' +
                                '<div class="button icon camera-action-button mouse-effect up" title="Cima"></div>' +
                                '<div class="button icon camera-action-button mouse-effect down" title="Baixo"></div>' +
                                '<div class="button icon camera-action-button mouse-effect left" title="Esquerda"></div>' +
                                '<div class="button icon camera-action-button mouse-effect right" title="Direita"></div>' +
                                '<div class="button icon camera-action-button mouse-effect zoom-in" title="zoom +"></div>' +
                                '<div class="button icon camera-action-button mouse-effect zoom-out" title="zoom -"></div>' +
    ...
    function updateCameras(cameras) {
        ...
        if ($('#cameraSelect').find('option').length < 2 && isAdmin() && !query.camera_ids) {
            ...
            var addCameraLink = $('<div class="add-camera-message">' +
                    '<a href="javascript:runAddCameraDialog()">Você ainda não configurou nenhuma câmera. Clique aqui para adicionar...</a></div>');
